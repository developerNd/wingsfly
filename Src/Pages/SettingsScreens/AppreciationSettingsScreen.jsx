...
const AppreciationSettingsScreen = ({navigation}) => {
  ...
  const loadAppreciation = async () => {
    try {
      const data = await appreciationStorageService.getAppreciationData();
      if (data) {
      ...
      console.error('Error loading appreciation:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUploadAudio = async () => {
    try {
      ...
      if (result && result[0]) {
        ...
        const audioDir = `${RNFS.DocumentDirectoryPath}/appreciation_audio`;
        ...
        const permanentFileName = `appreciation_${timestamp}.${extension}`;
        ...
        setUseAudio(true);
        setAppreciationText(''); // Clear text when switching to audio
  ...
  const handleSave = async () => {
    if (useAudio) {
      ...
      if (!appreciationText.trim()) {
        Alert.alert('Error', 'Please enter your appreciation message');
        return;
      }
    }
    ...
    setSaving(true);
    try {
      const result = await appreciationStorageService.saveAppreciationData({
        text: useAudio ? '' : appreciationText.trim(),
        audioFilePath: useAudio ? audioFilePath : null,
        audioFileName: useAudio ? audioFileName : null,
      });

      if (result.success) {
        Alert.alert('Success', 'Appreciation message saved successfully', [
          {
            text: 'OK',
        ...
        Alert.alert('Error', 'Failed to save appreciation message');
      }
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.headerWrapper}>
        <Headers title="Appreciation Message" navigation={navigation}>
        ...
        {!useAudio ? (
          <View style={styles.inputContainer}>
            <TextInput
              ...
              placeholder="Enter your appreciation message (e.g., 'Great job! You're unstoppable!')"
...
export default AppreciationSettingsScreen;