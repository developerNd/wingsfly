...
const MorningModeSettingsScreen = ({navigation}) => {
  ...
  const loadVoiceSettings = async () => {
    try {
      ...
      if (savedSettings) {
        ...
        setVoiceCommandEnabled(settings.enabled || false);
        setVoiceCommandText(settings.text || '');
      }
    } catch (error) {
  ...
  const saveContent = async () => {
    // Validate voice command settings if enabled
    if (voiceCommandEnabled) {
      if (!voiceCommandText.trim()) {
        Alert.alert(
          'Voice Command Required',
          'Please enter a voice command message or disable voice commands.',
        );
        return;
      }
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity
            ...
            <View
              style={[
                styles.toggleSwitch,
                voiceCommandEnabled && styles.toggleSwitchActive,
              ]}>
              <View
                style={[
                  styles.toggleThumb,
                  voiceCommandEnabled && styles.toggleThumbActive,
                ]}
              />
            </View>
          </TouchableOpacity>
        ...
        {/* Summary Section */}
        {(videos.length > 0 || audios.length > 0 || voiceCommandEnabled) && (
